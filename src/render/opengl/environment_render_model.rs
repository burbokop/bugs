...
pub struct OpenGlEnvironmentRenderModel {
    gl: Rc<glow::Context>,
...
impl OpenGlEnvironmentRenderModel {
    pub(crate) fn new(gl: glow::Context) -> Self {
        ...
        unsafe {
            ...
            let selected_light_color_position = gl
                .get_uniform_location(program, "selected_light_color")
                .unwrap();
...
impl<T> EnvironmentRenderModel<T> for OpenGlEnvironmentRenderModel {
    fn render(
        &mut self,
        ...
        unsafe {
            ...
            if self.next_texture.width != *view_port_size.w()
                || self.next_texture.height != *view_port_size.h()
            {
                let mut new_texture =
                    DemoTexture::new(gl, *view_port_size.w(), *view_port_size.h());
